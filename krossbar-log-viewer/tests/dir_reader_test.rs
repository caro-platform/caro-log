...
use krossbar_log_viewer::log_directory_reader::DirectoryReader;
...
fn create_log_files(path: &Path) {
    ...
    File::create(path.join("krossbar.log")).unwrap();
}
...
fn dir_reader_test() {
    ...
    let live_log_location: String = log_dir.path().join("krossbar.log").to_string_lossy().into();
    let mut read_result = DirectoryReader::read_dir_logs(&live_log_location).into_iter();

    let mut first_file = read_result.next().unwrap();
    assert_eq!(first_file.log_file_name, "krossbar_2015_02_17_23_16_09.log");

    first_file = read_result.next().unwrap();
    assert_eq!(first_file.log_file_name, "krossbar_2015_02_18_11_16_09.log");

    first_file = read_result.next().unwrap();
    assert_eq!(first_file.log_file_name, "krossbar_2015_02_18_23_16_09.log");

    first_file = read_result.next().unwrap();
    assert_eq!(first_file.log_file_name, "krossbar_2015_02_23_01_00_00.log");

    first_file = read_result.next().unwrap();
    assert_eq!(first_file.log_file_name, "krossbar.log");
